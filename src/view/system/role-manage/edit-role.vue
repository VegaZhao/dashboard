...
import { editRole } from '@/api/role-manage'
...
export default {
    ...
    editRoleOk () {
      return new Promise((resolve, reject) => {
        this.$refs.formCustom.validate(valid => {
          if (valid) {
            editRole(this.formCustom).then(res => {
              if (res) {
                resolve()
  ...
  watch: {
    roleInfo (val) {
      this.formCustom.roleId = val.roleId
      this.formCustom.roleName = val.roleName
      this.formCustom.roleDesc = val.roleDesc
      this.formCustom.permissions = val.permissions
    }
  },
  mounted () {
